...
class TestProject(unittest.TestCase):
    ...
    def testSetAlphaMatte(self):
        ...
        alphaMatte = alphaMatte.scaled(
            project.canvas().width(), project.canvas().height()
        )
        self.assertTrue(
            np.all(alphaMatte.byteView() == project.alphaMatte().byteView())
        )
    ...
    def testSetTrimap(self):
        ...
        self.assertTrue(
            np.all(
                trimapToRgba(trimap).byteView() == project.trimapPreview().byteView()
            )
        )
    ...
    def testSetNewBackground1(self):
        ...
        self.assertTrue(
            np.all(newBackground.byteView() == project.newBackground().byteView())
        )
    ...
    def testNew(self):
        ...
        self.assertTrue(
            np.all(
                np.full((newHeight, newWidth, 1), 0) == project.alphaMatte().byteView()
            )
        )
        self.assertTrue(
            np.all(
                np.full((newHeight, newWidth, 1), 160) == project.trimap().byteView()
            )
        )
        self.assertTrue(
            np.all(
                np.full((newHeight, newWidth), 0)
                == project.trimapPreview().byteView()[:, :, 3]
            )
        )
