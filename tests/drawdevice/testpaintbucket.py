...
class TestPaintBucket(unittest.TestCase):
    def testFill(self):
        ...
        self.assertTrue(
            np.all(trimapPreview.byteView()[:, 0:1] == Color.lightGreen.bgra())
        )
        self.assertTrue(
            np.all(trimapPreview.byteView()[4:6, :] == Color.lightGreen.bgra())
        )
        self.assertTrue(
            np.all(trimapPreview.byteView()[:, 9:11] == Color.lightGreen.bgra())
        )
