...
class SolverSettingsDialog(qtw.QDialog):
    ...
    def setupGeneralTab(self):
        ...
        self.laplacianNameComboBox = formWidget.addComboBox(
            "Laplacian", ["Closed-Form-Laplacian"], [None], laplacianToolTip
        )
        self.methodComboBox = formWidget.addComboBox(
            "Method", ["cgd", "vcycle"], [Method.cgd, Method.vcycle], methodToolTip, 0
        )
        self.radiusSpinBox = formWidget.addSpinBox(
            "Radius", 1, 5, 1, radiusToolTip, wrapping=False
        )
        self.epsilonSpinBox = formWidget.addDoubleSpinBox(
            "Epsilon",
            0.0000001,
            1,
            0.0000001,
        ...
        self.toleranceSpinBox = formWidget.addSpinBox(
            "Tolerance", 0, 99, 7, toleranceToolTip, "disabled", wrapping=False
        )
        self.lambdaSpinBox = formWidget.addSpinBox(
            "Lambda", 1, 100000, 100, lambdaToolTip, wrapping=False
        )
    ...
    def setupMethodTab(self):
        ...
        self.preconditionerComboBox = cgdFormWidget.addComboBox(
            "Preconditioner",
            ["none", "jacobi", "vcycle"],
            [Preconditioner.none, Preconditioner.jacobi, Preconditioner.vcycle],
            preconditionerToolTip,
            2,
        )
        vcycleFormWidget = FormWidget()
        vcycleGroupBox = VGroupBox("V-Cycle")
        self.kernelComboBox = vcycleFormWidget.addComboBox(
            "Kernel",
            ["linear", "gaussian"],
            [Kernel.linear, Kernel.gaussian],
            kernelToolTip,
            1,
        )
        self.preIterSpinBox = vcycleFormWidget.addSpinBox(
            "Pre-Iterations", 1, 1000, 1, preIterToolTip, wrapping=False
        )
        self.postIterSpinBox = vcycleFormWidget.addSpinBox(
            "Post-Iterations", 1, 1000, 1, postIterToolTip, wrapping=False
        )
    ...
    def setupConnections(self):
        ...
        self.buttonBox.button(qtw.QDialogButtonBox.RestoreDefaults).pressed.connect(
        ...
        self.preconditionerComboBox.currentIndexChanged.connect(
            lambda i: self.preconditionerChanged.emit(
                self.preconditionerComboBox.itemData(i)
            )
        )
    ...
    def emitHasVcycle(self):
        preconditioner = self.preconditionerComboBox.itemData(
            self.preconditionerComboBox.currentIndex()
        )
        method = self.methodComboBox.itemData(self.methodComboBox.currentIndex())
        hasVcycle = (
            method == Method.vcycle
            or method == Method.cgd
            and preconditioner == Preconditioner.vcycle
        )
        self.hasVcycle.emit(hasVcycle)

    def setupButtons(self):
        self.buttonBox = qtw.QDialogButtonBox(
            qtw.QDialogButtonBox.RestoreDefaults | qtw.QDialogButtonBox.Ok,
            qtc.Qt.Horizontal,
        )
