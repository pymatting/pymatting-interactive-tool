...
class Scaler(BaseWorker):
    def scale(self, project, newWidth, newHeight, keepAspectRatio):
        try:
            ...
            aspectRatioMode = (
                qtc.Qt.KeepAspectRatio if keepAspectRatio else qtc.Qt.IgnoreAspectRatio
            )
            if newWidth >= 1 and newHeight >= 1:
                canvas = project.canvas().scaled(
                    newWidth, newHeight, aspectRatioMode, transformationMode
                )
                alphaMatte = project.alphaMatte().scaled(
                    newWidth, newHeight, aspectRatioMode, transformationMode
                )
                trimapPreview = project.trimapPreview().scaled(
                    newWidth, newHeight, aspectRatioMode, transformationMode
                )
